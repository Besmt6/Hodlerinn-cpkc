...
# Admin password (simple protection) - MUST be set in environment
...
@api_router.get("/admin/sync/view-pdf")
async def view_sync_report_pdf():
    """View PDF report of the last sync results in browser (inline display)."""
    global sync_status
    ...
    # Create PDF in memory (same as export but with inline disposition)
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter, topMargin=0.5*inch, bottomMargin=0.5*inch)
    ...
    return StreamingResponse(
        buffer,
        media_type="application/pdf",
        headers={"Content-Disposition": "inline; filename=sync_report.pdf"}
    )
